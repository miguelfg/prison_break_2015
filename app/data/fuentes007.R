...
# Choose directory
getwd()
setwd("E:/Varios/R/Archivos/presos")
dir()
...
################################################################################
## INE site information

fileUrl <- "http://www.ine.es/pcaxisdl//t20/e245/p08/l0/02006.px"
download.file(fileUrl, destfile="./varios/02006.px")
dateDownloaded <- date()

fileUrl <- "http://www.ine.es/pcaxisdl//t20/e245/p08/l0/02003.px"
download.file(fileUrl, destfile="./varios/02003.px")
dateDownloaded <- date()

fileUrl <- "http://www.ine.es/pcaxisdl//t20/e245/p08/l0/02005.px"
download.file(fileUrl, destfile="./varios/02005.px")
dateDownloaded <- date()
...
# Series por Comunidad Autonoma. Población (españoles/extranjeros) por edad 
# (año a año), sexo y año  - 02003.px
px_02003 <- read.px("./varios/02003.px")
summary(px_02003)
px_02003$CONTENTS
ine_02003   <- as.data.frame(px_02003)
names(ine_02003)[length(ine_02003)] <- px_02003$CONTENTS
summary(ine_02003)
# save(ine_02003, file="./data/ine_02003.RData")

# Series por Comunidad Autonoma. Población extranjera por Nacionalidad, sexo y 
# año  - 02005.px
px_02005 <- read.px("./varios/02005.px")
summary(px_02005)
px_02005$CONTENTS
ine_02005   <- as.data.frame(px_02005)
names(ine_02003)[length(ine_02005)] <- px_02005$CONTENTS
summary(ine_02005)
# save(ine_02005, file="./data/ine_02005.RData")

# Series por Comunidad Autonoma. Población (españoles/extranjeros) por País de 
# Nacimiento, sexo y año - 02006.px
px_02006 <- read.px("./varios/02006.px")
summary(px_02006)
px_02006$CONTENTS
ine_02006   <- as.data.frame(px_02006)
names(ine_02006)[length(ine_02006)] <- px_02006$CONTENTS
summary(ine_02006)
# save(ine_02006, file="./data/ine_02006.RData")

table(ine_02006$País.de.Nacimiento)
table(ine_02006$comunidades)
table(ine_02006$Año)

ine_02006[ine_02006$País.de.Nacimiento=="Nueva Zelanda", ]

# List px files
ldply(list.files(pattern="px"))

scrape.1 <- function (x){ldply(list.files(pattern="csv"), function(filename) {
      dum <- read.csv(filename)
      dum$ruta <- as.numeric(substr(filename, 1, nchar(filename)-4))
      return(dum)
}

## Aggregate all csv files in a directory
## http://stackoverflow.com/questions/17609425/import-multiple-csv-files-in-r-while-retaining-their-names
...
my_px <- ldply(list.files(pattern="px"), function(filename) {
      dum <- read.px(filename)
      dum$ruta <- as.numeric(substr(filename, 1, nchar(filename)-4))
      dum_df   <- as.data.frame(dum)
      return(dum_df)
})

# list.files(path="./data", pattern="*.RData")
files_px <- list.files(pattern="*.px")

# Read all csv files on a disk location
data_px <- do.call(rbind, lapply(list.files(path=".", pattern="*.csv"), read.csv))

data_px <- do.call(print, lapply(list.files(path=".", pattern="*.px"), read.px))

################################################################################
## Anuario La Caixa
# http://www.anuarieco.lacaixa.comunicacions.com/java/X?cgi=caixa.le_DEM.pattern&CLEAR=YES
# En el link anterior se baja el fichero DatosAnuario_20130211120206.xls que contiene los 
# datos del anuario de La Caixa
# El fichero esta guardado primero en xls y luego desde LibreOffice con formato UTF-8
# Se han quitado las provincias y mejorado los nombres de las variables

anuario <- read.csv("./varios/AE13_Comunidades_Completo.csv", header=TRUE, 
                    fileEncoding="UTF-8", stringsAsFactors=FALSE, sep=";")

summary(anuario)
str(anuario)
names(anuario)

save(anuario, file="anuario.RData")
